...
```cpp
int bad = 10 / 0; // UB
double bad2 = 10.0 / 0.0; // Defined, result is infinity -- IEE754
```
...
**Your answer:**
This is a bad comparison because these floating-point types aren't stored exactly as-is internally so when you compare two floating-point types thinking they would be the same they can certainly not be. In this case we would use epsilon-based comparisons.
```cpp
double x = 0.1 + 0.2;
double y = 0.3''
bool is_equal = std::abs(x - y) < 1e-9;
```
...
**Your answer:**
Division by zero! UB! This is why Short-circuit evaluation is helpful and will prevent this if the order is right.
...
**Your answer:**
Rather than having multiple variables for different options, you can have one integer that depending on their bit pattern represent different options.
...
**Your answer:**
Parentheses clearly show what operations should be done first. `*` has higher precedence than `+`.
...
**Your answer:**
1. This is because statements that are meant for the `if` statement will not be enclosed within that if statement. Only the first statement after an `if` statement with no braces is container within that `if` block.
2. This form allows initialization for the entire life-time of the `if` block.
...
**Your answer:**
The ternary operator allows for a short form evaluation of a condition. It is an expression because it produces a value.
```cpp
...
**Your answer:**
`break` prevents fall-through, if you leave it out the statements under will continually be executed until the end or a `break` statement is found. `[[fallthrough]]` tells the compiler to not warn about fall-through, as it may be intentional.
